...
class SuperPoint:
    ...
    def __init__(
        ...
        self.height = height
        self.width = width
        self.keep_kp_best = keep_kp_best
        self.weights_root_dir = Path(EXPER_PATH, "saved_models")
        self.weights_dir = Path(self.weights_root_dir, self.weights_name)



    def extract_superpoint_keypoints_and_descriptors(self, keypoint_map, descriptor_map, keep_k_points):
    ...
    def preprocess_image(self, img_file, img_size, resize_image):
    ...
    def get_superpoint_kp_desc(self):

        img_size = (self.width, self.height)

        graph = tf.Graph()
        with tf.compat.v1.Session(graph=graph) as sess:
            tf.compat.v1.saved_model.load(sess, [tf.saved_model.SERVING], str(self.weights_dir))
            ...
            img, img_orig = self.preprocess_image(self.img_file, img_size, self.resize_image)
            out = sess.run(
            ...
            kp, desc = self.extract_superpoint_keypoints_and_descriptors(keypoint_map, descriptor_map, self.keep_kp_best)
        return kp, desc


if __name__ == "__main__":
    kp_super, desc = SuperPoint(img_file="/content/drive/MyDrive/ColabNotebooks/superpoint/data/val2014/COCO_val2014_000000000164.jpg", resize_image=False).get_superpoint_kp_desc()
